...
productsClient.get()
              .then(products => {
                const manFilteredProducts = products.filter(product => product.category === 'man');
                manProducts = printProducts(manFilteredProducts, productsContainer, productCardTemplate);
              })
              .catch(err => {
                console.log(err);
              })
 ...
 search.addEventListener('input', (event) => {
    ...
    manProducts.forEach(({product, element}) => {
